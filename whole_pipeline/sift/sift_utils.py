#These are the functions we agreed to use as built in functions
import numpy as np
import cv2
import math
        ...
        if region_y > 0 and region_y < image_shape[0] - 1:
            for j in range(-radius, radius + 1):
                ...
                if region_x > 0 and region_x < image_shape[1] - 1:
                    ...
                    weight = math.exp(weight_factor * (i ** 2 + j ** 2))  # constant in front of exponential can be dropped because we will find peaks later
...
def generateDescriptors(keypoints, gaussian_images, window_width=4, num_bins=8, scale_multiplier=3, descriptor_max_value=0.2):
    ...
    for keypoint in keypoints:
        ...
        cos_angle = math.cos(np.deg2rad(angle))
        sin_angle = math.sin(np.deg2rad(angle))
