...
cdef class Feature:
    def __cinit__(self, list rect_lists):
        self.rect_lists = rect_lists
        pass

    cdef float compute_feature(self, np.ndarray[np.float64_t, ndim=2] integral_image_window, int scale):
        cdef int value = 0
        cdef int x, y, rect_width, rect_height
        cdef float rect_weight
